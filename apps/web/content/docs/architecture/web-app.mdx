...
The `(app)` route group wraps all app pages in `AppLayout` from `@workspace/ui`, passing locale-aware `Link` and `usePathname` from `@workspace/i18n/navigation`. The sidebar, header, and navigation all come from the shared UI package. See [UI Package](/docs/architecture/ui-package) for component details.
